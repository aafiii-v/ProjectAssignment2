const Content = ({ beans }) => {
  return (
    <div className="pt-10 px-4 pb-10 bg-[#7a3e1d] min-h-screen text-black">
      {beans.length === 0 ? (
        ...
        beans.map((bean) => (
          <div
            key={bean.id}
            className="max-w-4xl mx-auto bg-[#c15826] rounded-xl shadow-md p-4 mb-6 flex flex-col md:flex-row items-center gap-4
                     transform transition duration-300 ease-in-out hover:scale-105 hover:shadow-xl hover:bg-[#d86c35]"
          >
            <img
              src={bean.gambar}
              alt={bean.nama}
